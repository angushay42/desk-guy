/**
 * RoboEyes adapted from TFT_RoboEyes (Yousef Tech), 
 * adapted from RoboEyes (FluxGarage).
 * 
 * Customised to work with ESP-IDF with no Arduino dependencies.
 */

#ifndef ESP_TFT_ROBOEYES_H
#define ESP_TFT_ROBOEYES_H
...
class ESP_TFT_RoboEyes {
  public:
    ...
    // Constructor
    // ---------------------------
    ESP_TFT_RoboEyes(Display &display, bool portrait = true, int rotations = 1) 
    {
        disp = &display;
    ...
    void begin(uint8_t frameRate = 50) 
    {
        // Allocate and create the sprite (off-screen buffer)
        sprite = new Sprite(disp);
