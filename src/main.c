...
#define DC     GPIO_NUM_12 // Data Line
...
static esp_err_t scale_rgb(uint8_t src, uint8_t *dest, size_t size) {
    ...
    t_max = 255.0;
    *dest = (uint8_t) round(((double) src * t_max) / r_max);
...
void tft_display_test_write(esp_lcd_panel_handle_t handle) {
    for (size_t row = 0; row < (size_t) LCD_H_RES; row += LCD_MAX_LINES) 
    {
...
void uart_test(void) {
    ...
    const char *test = "HELLO HELLO HELLO\r\n";
    for (;;) {
